...
void scale_c_k21(double *C, int M, int N, int LDC, double beta) {
    if (beta == 0.0) {
...
static inline void micro_kernel_k21(int k_size,
                                  double alpha,
                                  const double *A,
                                  const double *B,
                                  double *C,
...
static inline void pack_a_k21(const double *A, double *A_packed, int LDA, int M, int K) {
...
static inline void pack_b_k21(const double *B, double *B_packed, int LDB, int K, int N) {
    int np = (N + NR - 1) & -NR;  // Round up to multiple of NR
    
    // Pack complete K x NR blocks
    for (int j = 0; j < N - NR + 1; j += NR) {
        for (int k = 0; k < K; k++) { //vector dependence prevents vectorization
            for (int jj = 0; jj < NR; jj++) { // unrolled by 4
...
static void macro_kernel_k21(int M, int N, int K,
                           double alpha,
...
// Main DGEMM function
void mydgemm_cpu_v21(int M, int N, int K,
    ...
    for (int n_start = 0; n_start < N; n_start += N_BLOCK) {
        ...
        for (int k_start = 0; k_start < K; k_start += K_BLOCK) {
            ...
            // Pack B block
            pack_b_k21(&B(k_start, n_start), B_packed, LDB, k_size, n_size);
            
            for (int m_start = 0; m_start < M; m_start += M_BLOCK) {
                int m_size = min(M_BLOCK, M - m_start);
                
                // Pack A block
                pack_a_k21(&A(m_start, k_start), A_packed, LDA, m_size, k_size);
                
                // Compute C block
                macro_kernel_k21(m_size, n_size, k_size, alpha,
                               A_packed, B_packed,
